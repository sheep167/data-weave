...
function CompareModeBanner() {
    ...
    return (
        ...
        >
            ...
            <span className="text-xs text-indigo-300 font-medium">{t("review.viewing")}</span>
...
const SchemaCanvasInner = observer(() => {
    ...
    const [nodes, setNodes, onNodesChange] = useNodesState(toNodes(schema.schema.entities));
    const [edges, setEdges, onEdgesChange] = useEdgesState(toEdges(schema.schema.relationships));

    // Sync schema store → React Flow nodes/edges (respects compare mode)
    useEffect(() => {
        const dispose = reaction(
            () => {
                const isAfter = reviewStore.compareMode === "after" && reviewStore.result !== null;
                const source = isAfter ? reviewStore.result!.proposedSchema : schema.schema;
                const originalEntityIds = new Set(schema.schema.entities.map((e) => e.id));
            ...
            ({ entities, relationships, _isAfter, _originalEntityIds, _originalFieldCounts }) => {
                    ...
                    return entities.map((e) => {
                        ...
                        if (_isAfter) {
                            if (!_originalEntityIds.has(e.id)) {
                                _diff = "new";
                            } else if ((_originalFieldCounts.get(e.id) ?? 0) !== e.fields.length) {
                                _diff = "modified";
                            }
                        }
                        return {
    ...
    const isCompareAfter = review.compareMode === "after" && review.result !== null;
