...
import type { SqlType, Cardinality, FieldConstraints } from "@data-weave/shared";
...
function DeleteEntityButton({ entityId, entityName }: { entityId: string; entityName: string }) {
    const { schema, ui } = useStore();
    const { t } = useTranslation();
...
const NewRelationshipPanel = observer(() => {
    ...
    const sourceEntity = schema.schema.entities.find((e) => e.id === sourceEntityId);
    const targetEntity = schema.schema.entities.find((e) => e.id === targetEntityId);

    // Auto-pick PK fields
    const sourcePK = sourceEntity?.fields.find((f) => f.constraints.primaryKey);
    const targetFK =
        targetEntity?.fields.find((f) => f.name.includes("_id")) ?? targetEntity?.fields[0];
...
const EditRelationshipPanel = observer(() => {
    ...
    const sourceName = schema.schema.entities.find((e) => e.id === rel.sourceEntityId)?.name ?? "?";
    const targetName = schema.schema.entities.find((e) => e.id === rel.targetEntityId)?.name ?? "?";
